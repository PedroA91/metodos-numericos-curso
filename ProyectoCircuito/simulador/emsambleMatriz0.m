function [G,J] = emsambleMatriz0(Dato,gl,n_E,n_nodos)
	...
	% Esta funciones emsamblas las matrices para el circuito equivalente
	...
	for k = 1:n_E
		...
		elseif S.Elemento{k,1}(1,1) == 'L'
			...
			% Este renglon se agrego
			G(n_nodos+n_ex,n_nodos+n_ex) = G(n_nodos+n_ex,n_nodos+n_ex)-...
			(1e9)*str2num(S.Parametros{k,2});
			
		elseif S.Elemento{k,1}(1,1) == 'C'
			
			val = S.Parametros{k,1};
			G(np,np) = G(np,np)+(1e9)*str2num(S.Parametros{k,2});
			G(nn,nn) = G(nn,nn)+(1e9)*str2num(S.Parametros{k,2});
			G(nn,np) = G(nn,np)-(1e9)*str2num(S.Parametros{k,2});
			G(np,nn) = G(np,nn)-(1e9)*str2num(S.Parametros{k,2});
